...
export default function ArtistDashboard() {
  ...
  return (
    <div>
      ...
      <h1 className="mt-2 font-display text-2xl font-semibold text-paper sm:text-3xl">Dashboard</h1>
      <p className="mt-2 text-sm text-muted">Upload a track, then group any of your uploads into an album.</p>

      <div className="mt-8 grid gap-6 lg:grid-cols-2 lg:gap-8">
        <section className="animate-fade-in-up rounded-xl border border-hairline bg-surface p-4 transition-shadow hover:shadow-lg hover:shadow-black/10 sm:p-5">
          ...
          <form onSubmit={handleUpload} className="mt-4 flex flex-col gap-4">
            <label className="flex flex-col gap-1.5">
              ...
              <input
                ...
                className="rounded-lg border border-hairline bg-surface-raised px-4 py-2.5 text-sm text-paper outline-none transition-colors focus:border-amber"
            ...
            <label className="flex flex-col gap-1.5">
              ...
              <input
                ...
                className="rounded-lg border border-hairline bg-surface-raised px-3 py-2.5 text-sm text-paper file:mr-3 file:rounded-full file:border-0 file:bg-amber file:px-3 file:py-1 file:text-xs file:font-semibold file:text-ink file:transition-opacity hover:file:opacity-90 sm:px-4"
              />
            </label>

            {uploadError && <p className="animate-fade-in text-sm text-danger">{uploadError}</p>}

            <button
              type="submit"
              disabled={uploading}
              className="press rounded-full bg-amber px-4 py-2.5 text-sm font-semibold text-ink transition-opacity hover:opacity-90 disabled:opacity-60"
            >
            ...
            <ul className="mt-2 flex flex-col gap-1">
              {uploadedTracks.map((t, i) => (
                <li
                  ...
                  className="stagger-in truncate rounded-lg px-2 py-1 text-sm text-paper transition-colors hover:bg-surface-raised"
                >
                  {t.title}
                </li>
              ))}
        ...
        <section className="animate-fade-in-up rounded-xl border border-hairline bg-surface p-4 transition-shadow hover:shadow-lg hover:shadow-black/10 sm:p-5">
          ...
          <form onSubmit={handleCreateAlbum} className="mt-4 flex flex-col gap-4">
                ...
                <div className="mt-2 flex max-h-56 flex-col gap-1 overflow-y-auto">
                  {uploadedTracks.map((t, i) => (
                    <label
                      key={t.id}
                      style={{ '--stagger-index': i }}
                      className="stagger-in flex items-center gap-3 rounded-lg px-3 py-2 transition-colors hover:bg-surface-raised"
                    >
                      <input
                        type="checkbox"
            ...
            {albumError && <p className="animate-fade-in text-sm text-danger">{albumError}</p>}
            {albumSuccess && <p className="animate-fade-in text-sm text-teal">{albumSuccess}</p>}
